...
// Narration/description — the templates have a dedicated SECOND ruled line
// below the narration line, so text must flow DOWNWARD (top-anchored) across
// both lines: line 1 rests on the narration rule, line 2 on the continuation
// rule. `top` is positioned one line-gap above the narration rule and
// lineHeight matches the ruled-line spacing, so each printed line lands on
// its own rule; clamps at 2 lines so it never grows the voucher. Stored
// narration is untouched; full text remains available via the title attr.
...
export function PaymentVoucherPrintPreview({ voucher, template }: { voucher: PrintableVoucher; template: string }) {
  ...
  return (
    <VoucherPrintSheet template={template} alt="Payment voucher template" aspect="1568 / 1014" signature={voucher.signatureData}>
      ...
      <Typography title={voucher.description || ''} sx={{ ...voucherNarrationFieldSx, left: '23.2%', top: '52.4%', width: '70%', lineHeight: 2.4, maxHeight: '4.8em' }}>{voucher.description || ''}</Typography>
...
export function ReceiptVoucherPrintPreview({ voucher, template }: { voucher: PrintableVoucher; template: string }) {
  ...
  return (
    <VoucherPrintSheet template={template} alt="Receipt voucher template" aspect="1600 / 1035" signature={voucher.signatureData}>
      ...
      <Typography title={voucher.description || ''} sx={{ ...voucherNarrationFieldSx, left: '21.8%', top: '52.4%', width: '71.5%', lineHeight: 2.4, maxHeight: '4.8em' }}>{voucher.description || ''}</Typography>
...
export function JournalVoucherPrintPreview({ voucher, template }: { voucher: PrintableVoucher; template: string }) {
  ...
  return (
    <VoucherPrintSheet template={template} alt="Journal voucher template" aspect="1559 / 1009" signature={voucher.signatureData}>
      ...
      <Typography title={voucher.description || ''} sx={{ ...voucherNarrationFieldSx, left: '18%', top: '68.3%', width: '75%', lineHeight: 1.55, maxHeight: '3.1em' }}>{voucher.description || ''}</Typography>
